...
replaces = [
    ...
    ('"测量结果:"', TR("测量结果:")),
    ('f"  节目响度:      {self.results.integrated:+.2f} LUFS"', 'QCoreApplication.translate("ReportExporter", "  节目响度:      {val:+.2f} LUFS").format(val=self.results.integrated)'),
    ('f"  最大短时响度:  {self.results.short_term:+.2f} LUFS"', 'QCoreApplication.translate("ReportExporter", "  最大短时响度:  {val:+.2f} LUFS").format(val=self.results.short_term)'),
    ('f"  最大瞬时响度:  {self.results.momentary:+.2f} LUFS"', 'QCoreApplication.translate("ReportExporter", "  最大瞬时响度:  {val:+.2f} LUFS").format(val=self.results.momentary)'),
    ...
    # CSV rows
    ('["节目响度", f"{self.results.integrated:.4f}", "LUFS", "-23.0",', '[QCoreApplication.translate("ReportExporter", "节目响度"), f"{self.results.integrated:.4f}", "LUFS", "-23.0",'),
    ('["最大短时响度", f"{self.results.short_term:.4f}", "LUFS", "-23.0", "-"],', '[QCoreApplication.translate("ReportExporter", "最大短时响度"), f"{self.results.short_term:.4f}", "LUFS", "-23.0", "-"],'),
    ('["最大瞬时响度", f"{self.results.momentary:.4f}", "LUFS", "-", "-"],', '[QCoreApplication.translate("ReportExporter", "最大瞬时响度"), f"{self.results.momentary:.4f}", "LUFS", "-", "-"],'),
